...
```
The robustness is controlled by the parameter of the Huber loss function. This can be chosen by cross-validation which takes a long time, but gives good results. Altenratively, we supply our own parameter $C*sd*rate$ where $C$ is a constant, rate is the optimal rate for the tuning parameter. See Fan et al.(2017) <https://arxiv.org/abs/1612.08490>. $sd$ is the standard deviation of the residuals after running the OLS version of the Huber's estimator for the parameter to be estimated.   
