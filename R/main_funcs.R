...
#' @param lin.reg an \emph{optional} boolean, specifying whether or not to assume that we have a linear regression model (TRUE) or a logit model (FALSE) structure. Default is TRUE.
#' @param K.factors a \emph{optional} number of factors to be estimated. Otherwise estimated internally.
#' @param max.iter The maximum number of passes over the data for each value of regularization parameter for model fitting. Default \code{10^3}.
...
#' @export
farm.select <- function (Y, X,  loss=c("mcp", "scad", "lasso"), verbose = TRUE, robust = FALSE,  lin.reg = TRUE,K.factors= NULL, max.iter=1000,nfolds = 10){
  loss = match.arg(loss)
  #error checking
  X = t(X)
  if(NCOL(X)!=NROW(Y)) stop('number of rows in covariate matrix should be size of the outcome vector')
    output.final = farm.select.adjusted(Y, X,   loss=loss, robust,lin.reg,K.factors, max.iter,nfolds )
    if(verbose){output.final.call = match.call()
    cat("\nCall:\n")
    print(output.final.call)
    cat("\n Factor Adjusted Robust Model Selection \n")
    cat(paste("loss function used: ",  loss, "\n",sep = ""))
    cat(paste("\np = ", NROW(X),", n = ", NCOL(X),"\n", sep = ""))
...
farm.select.adjusted <- function (Y, X,   loss,robust ,lin.reg,K.factors, max.iter,nfolds ){
  ...
  if(lin.reg){
   output.chosen = farm.select.temp ( X.res, Y.res, loss=loss, max.iter,nfolds)
  }else{
    F_hat =  output.adjust$F_hat
    output.chosen = farm.select.temp ( X.res, Y.res, loss=loss, max.iter,nfolds ,factors = F_hat )
  }
...
#'
#' output = farm.res(X, K.factors = 1) #specify number of factors
...
farm.res<- function(X ,K.factors = NULL, robust = FALSE) {#, robust.cov = FALSE) {
  ...
  list(X.res = X.res, nfactors = K.factors, factod = F_hat, loadings  = Lambda_hat)
